...
struct ReceitaBuscaView: View {
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            TextField("Pesquise sua receita",text: $textoBusca)
            .font(.system(size: 16))
            
            Button {
                iniciarBuscaPorVoz()
            } label: {
                Image(systemName: estaOuvindo ? "mic.fill" : "mic")
                .font(.system(size: 16))
                .foregroundStyle(.secondary)
            }
            .accessibilityLabel(estaOuvindo ? "Parar pesquisa por voz" : "Pesquisar por voz")
        }
